...
> 本文件记录开发过程中的讲解、问答与踩坑，兼作知识记录与工作日志。
...
20. 复盘与修订记录（2026-09-09）

> 注：编号 7 原为"阶段性自测清单"，2026-09-09 按笔记定位要求移除；正文章节编号不重排，以免破坏既有交叉引用（目录、README、正文多处按编号引用）。
...
- 何时该从 KF 升级为真 EKF：判据见 15.3 的"何时变 EKF"。
...
- **本次修订**：① README v2.0 重构（三题并列结构 + 状态总览 + FAQ + 修订记录）；② .gitignore 白名单让证据录屏入库（修复 README 死链）；③ 19.1 验收表与 19.6/19.7 同步；④ 补充 LICENSE。
...
### 20.2 笔记定位修订（2026-09-09）

- 按使用反馈移除全部"自测清单"类区块（原 §7 阶段性自测清单、原"全量面试自测清单"小节、原收尾核对清单章节）与所有"面试"指向性表述；
- 笔记定位回归为**知识记录 + 工作日志**，不再出现"可用于面试 / 面试谈资"类提示；
- 正文章节编号不重排：编号 7 空缺（见 0. 目录注），既有交叉引用（§16.3/§17/§18/§19.4 等）保持有效。
