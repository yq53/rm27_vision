...
7. 自测清单（题1 阶段，全量清单见 20.2）
8. 下一步（题1 阶段的历史待办）
9. 题2 tracker：术语表与学习路线（v1 已完成）
10. L1 坐标系与针孔相机模型（题2 数学地基）
11. L2 正投影演示（题2 数学地基·代码版）
12. L3 四角点 v1：corner_demo（题2 基础）
13. L4 PnP 概念问答与 pnp_demo（题2 核心）
14. L5 概率统计速补（EKF 前置）
15. L6 卡尔曼心智模型与 EKF 决策
16. L7 ArmorEKF 与 tracker_demo（题2 v1 完成）
17. L8 v2 灯条精定位（探索原型，已冻结）
18. 模型域差实验（Plan A 负结果存档）
19. 题3 ROS2 接入与可视化（P0/P1 + 真实相机验证完成）
20. 复盘修订与全量自测清单（2026-09-09）
21. 收尾核对清单（提交前必查）
...
## 15. L6 卡尔曼心智模型与 EKF 决策（题2 核心概念）

### 15.1 卡尔曼心智模型（三量 + 两空间融合）
...
### 15.4 为什么 K 的分子是 $\bar{P}H^{\mathsf T}$（关键代数）
...
## 18. 模型域差实验（Plan A：暗化增强微调，2026-09-07）
...
| **真实相机验证** | ✅ 手机 IP Webcam 实拍验证通过（2026-09-08 首次、09-09 录屏定稿，见 19.6/19.7） |
| 交付文档/录屏 | ✅ README v2.0 + 截图/录屏证据（2026-09-09 复盘修订） |

> 2026-09-09 复盘修订：本表原"真实相机 ❌ 待做 / 交付 ⏳"两行已按 19.6/19.7 的实际完成情况同步。
...
- **待办**：README 补截图/录屏证据；可选重构（solveArmorPosition 抽库）；面试演练 + 多端备份。

### 19.7 真实相机录屏证据定稿（2026-09-09）
...
- **本次修订**：① README v2.0 重构（三题并列结构 + 状态总览 + FAQ + 修订记录）；② .gitignore 白名单让证据录屏入库（修复 README 死链）；③ 19.1 验收表与 19.6/19.7 同步；④ 补充 LICENSE；⑤ 本清单扩展。
...
### 20.2 全量面试自测清单（答题出处=各章，先口述再对笔记）

**A. 题1 / 现代 C++ / OpenCV DNN**
- [ ] `#pragma once` 与宏保护等价（§4.1）
- [ ] `explicit` 解决什么问题、何时必须加（§4.2）
- [ ] 声明 vs 定义；头文件为什么不放实现（§4.3）
- [ ] 一个成员函数该不该加 static 的判据（§4.4）
- [ ] IoU 公式 + NMS 贪心流程；conf 与 nms 两阈值各控制什么（§4.6）
- [ ] 头文件为什么禁止 `using namespace`；`rm_vision::` 前缀意义（§4.5）
- [ ] Google 风格常量/成员/局部变量命名差异（§4.7）
- [ ] letterbox 缩放+pad 与坐标还原（减 pad 除 ratio）的互逆关系（§5.2）
- [ ] blob 的 NCHW 排布、swapRB、1/255 各做什么（§5.4）
- [ ] YOLO 输出两种排布 `(1,C,N)/(1,N,C)` 与两个下标公式（§5.9/5.13）
- [ ] 解析为什么算 argmax 而非硬编码类别（§5.11/5.14）
- [ ] `atoi` vs `stoi`；`const T&` 捕获异常的理由（§6.2/6.8）
- [ ] `unique_ptr` 让对象活过 try 块的原理（§6.9）

**B. 题2 / 数学 / PnP / 滤波**
- [ ] 世界→相机→像素的翻译链与针孔模型（§10.1/10.4）
- [ ] 内参 K 各元素含义；fx 与 FOV 关系；齐次坐标把平移写进矩阵（§10.5/10.6/19.6）
- [ ] PnP 为什么需要四角点而非轴对齐 bbox（§12.1）
- [ ] solvePnP 输入输出；IPPE 镜像歧义 + r₃.z>0 破镜（§13.2/13.4）
- [ ] 重投影误差含义与 >10px 拒绝（§13.3）
- [ ] 为什么状态是 pos+vel；高斯/协方差直觉（§14.4）
- [ ] 卡尔曼五步；x/P 原位更新（x̄/P̄ 是阶段语义非两个变量）（§15.2/16.2）
- [ ] 本项目为何"EKF 框架、线性退化为 KF"即可；何时必须真 EKF（§15.3/15.6）
- [ ] Q 大→P 涨→更信测量；R 大→更信预测（§16.2）
- [ ] dt 现造 F/Q；漏检帧 predict-only 的意义（§16.2/16.4）
- [ ] 单目标=挑面积最大的简化代价；MOT 需要什么（§16.3）
- [ ] v2 灯条精定位为何冻结（配对率 ~7%）；三级容错设计（§17.4/17.6）
- [ ] Plan A 压暗微调为何是负结果（灯条同暗≠真实暗场"灯条亮背景黑"）（§18.2/18.3）

**C. 题3 / ROS2 / 工程与证据**
- [ ] Node/Publisher/Timer/Parameter；spin 作用（§19.3）
- [ ] Header 的 stamp/frame_id 用途（§19.3）
- [ ] cv_bridge Mat↔Image（§19.3）
- [ ] QoS：图像 best_effort、状态 reliable 的理由（§19.3）
- [ ] CycloneDDS 大图像丢失 → 换 FastDDS；RMW 排查顺序（§19.4）
- [ ] 自定义消息为何必须独立接口包；member_of_group 须在顶层（§19.2）
- [ ] 竖屏旋转元数据 → PnP 镜像假解（z<0）原理（§19.6）
- [ ] computeK 的 HFOV≈72° 近似内参适用边界（量级可信/绝对精度需标定）（§19.6）
- [ ] 物理闸门 z>0.2 & dist<20m 防什么（§19.6）
- [ ] 三题可复现数字：72.5% / 0.042→0.020 m / d≈0.45~1.05 m（§16.1/19.7）
- [ ] 三个"诚实叙事"：demo.avi 永不进训练、Plan A 负结果、v2 冻结（§17/18）

## 21. 收尾核对清单（提交前必查）

- [ ] `git push origin main`，GitHub 上 README/截图可正常浏览
- [ ] QQ 群 469113615 提交仓库地址
- [ ] 报名表发送至 2826223614@qq.com（截止 **2026-09-16 22:00**）
- [ ] 线下考核备份（网盘 / U盘）
- [ ] 面试前过一遍 20.2 清单
- [ ] 后续代码层可选项：CMake 注释同步、solveArmorPosition 抽库、棋盘标定、仿真器接入、MOT/预测
