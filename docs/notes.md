...
> **声明**：本文正文由 AI 协助成文；其中的路线选择、负结果判断与取舍由本人决策。
> 代码实现同样由 AI 工具生成，详见仓库 `README.md` 顶部的「代码来源与用途声明」。
...
> | [`log.md`](log.md) | 为什么这么做、当时怎么想、试错过程（**过程日志，24 章，章节号稳定**） | 想溯源推导与踩坑细节 |
>
...
> **可用范围（重要）**：命令行演示程序（`armor_demo` / `corner_demo` / `pnp_demo` / `tracker_demo` /
> `lightbar_demo`）**只支持 bbox** —— 它们是**学习阶段**写的，那时全程走 bbox 路线；
> **pose 模型是最后优化阶段才引入的**，因此只出现在两处：**题3 的完整链路（节点 / launch）** 与
> **题2 的「离线评测基础设施（`eval_demo`）」**（第 6 个参数 `bbox|pose`，687 帧 A/B 表就是它跑的）。
...
| 编辑器里 `04_hik` 报找不到 `MvCameraControl.h` | **clangd 只从源文件往上找第一个 `compile_commands.json`，且不合并多个**：根目录那个是题3 的 DB（题3 通过 `add_subdirectory` 带了 01/02，所以它们正常；04_hik 是独立工程，不在里面）→ 在每个工程目录下各放一条软链（`setup.sh` 已自动做） | `README.md`「构建」/ FAQ |
...
- 想**快速建立整体认识** → 本文件
- 想**溯源推导 / 看踩坑过程 / 看负结果原始记录** → [`log.md`](log.md)（24 章，章节号稳定）
