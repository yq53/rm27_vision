...
**仍未做（均为加分项，按当前判断的价值排序）**：

- **④ PnP 降自由度**：固定 pitch/roll，只解 yaw（1D）—— 现在 6 自由度全解，俯仰/翻滚在"小目标单板"上是纯噪声源；
- **R/Q 噪声建模升级**：按 YPD→XYZ 雅可比传播协方差 + 分段白噪声（当前 Q/R 为常值）；
- **多目标 / 整车 EKF**：当前只跟"面积最大的单板"，两块板交替最大时会跳目标；
- **装甲数字识别**：区分敌我与选板优先级的前置能力；
- **棋盘格内参标定**：替换 HFOV≈72° 近似，让距离从"量级可信"升级为"绝对可信"；
...
| 内容 | README | 本笔记 |
|---|---|---|
| `scripts/demo.sh` 一键启动（手工版保留） | ✅ 题3「一键启动脚本」 | ✅ 本节 |
...
- **README 有、笔记无**：使用体验类内容（快速开始、预期效果、FAQ 排错表、`demo.sh` 用法）——
  笔记不承担"操作手册"职责。
