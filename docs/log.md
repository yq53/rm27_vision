...
| **"clone 者照 README 复制粘贴能否看到效果"的端到端验证**（2026-09-12）：在模拟干净机器（深层目录，所有 ORT 候选都落空）里逐步跑 README —— ① bbox 零依赖可用、② pose 在没 ORT 时 1 秒内明确报错退出、解出 ROS 官方 `onnxruntime-vendor` 包当 `ORT_DIR` 后 ② 也可用；据此把 ORT 的推荐获取方式改为 **apt vendor 包优先**，并把 README 改成"先跑 ①、再按需升级 ②" | ✅ README v3.11 / 第三步 | ✅ 本节 |
