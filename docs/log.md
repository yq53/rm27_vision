...
**本文件是「过程日志」（log）**：按时间顺序记录学习过程——概念问答、数学推导、踩坑、负结果与决策依据。
**它记录的是"当时怎么想"，所以刻意保持原样**（包括走过的弯路与后来的更正），不为了好看而回头删改。

**三份文档的分工**

| 文档 | 回答什么 | 特点 |
|---|---|---|
| `README.md` | 怎么跑、该看到什么、指标是多少 | 面向使用者；每个论断都能在本日志或 `notes.md` 找到依据 |
| `notes.md`（**按主题归纳的总结**） | 这个系统由什么组成、关键约定是什么 | 面向速查；每条结论后面都标了回到本文件的出处（`log.md §X.Y`） |
| 本文件 `log.md` | 为什么这么做、当时怎么想、试错过程 | 面向溯源；**过程原始记录，章节号稳定不变** |

三重交叉验证（README ↔ notes ↔ log）的结论见 **§22**。
...
## 22. README / notes / log 三重交叉验证（2026-09-11，原「双向对照」）

目的：README 面向"**怎么跑、该看到什么、指标是多少**"，本日志面向"**为什么、当时怎么想、试错过程**"，`notes.md` 是两者的**主题归纳**。
...
### 22.2 本日志 → README（日志结论是否已反映到 README）
...
| README「真实相机验证证据」/ 考核对照表 | "走通**真实相机**路线" | 实际是**手机 IP Webcam 对着笔记本屏幕拍摄**（屏幕在放 `demo.avi`，属二次成像）——不是真实场地/实车验证，也不是工业相机 | README 已改为「**相机通路验证**」并加"如实说明这是怎么拍的"引用块（v3.14） |
...
### 23.8 落地：EKF 的时间步长从"名义帧率"改为实测 dt（2026-09-15）

- **背景**：`dt_` 原先取 `1 / fpsHint()`（名义 33.3 ms），而成员 `last_img_time_` 只是个**没被使用**的残留（初值 `now() - 1s`，一看就是准备算实测 dt 用的）。
- **改动**（`03_visualization/src/armor_tracker_node.cpp`，单文件）：
  1. 新增常量 `kDtMinSec = 1 ms`、`kDtMaxSec = 200 ms`；
  2. `onTimer()` 开头用 `steady_clock` 两次回调的**真实间隔**算 `dt_`，`std::clamp` 夹取后更新 `last_img_time_`；
  3. 把 `ekf_.predict(dt_)` **移到读帧之前**——这样某帧 `read()` 失败（原实现直接 `return`）时，滤波器的时间照常推进，不再丢一个时间步长。
