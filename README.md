...
> ## ⚠️ 代码来源与用途声明（请先阅读）
>
> - **本仓库的代码实现由 AI 编程工具 DSH 生成**；本人负责拆解题目、确定技术路线与方案取舍、逐条审查代码
>   并提出修改意见、验收与纠错（手写内容仅为少量注释与配置调整）。**笔记正文亦为 AI 协助成文。**
...
- **代码实现：由 AI 编程工具 DSH（DeepSeek Harness）生成。**
...
- 本仓库**仅作个人学习记录**，不是成熟项目，也**不作为生产级或可直接复用的工程实现**；
  仓库中的指标、参数与结论均**未经生产环境验证**，引用前请自行验证。
- 考核题面未禁止使用 AI 工具，此处**如实标注实际分工**。本人对算法与工程的理解过程记录在
  `docs/log.md`，**欢迎针对其中任何一条提问**。

**第三方资源**

- 四关键点模型：直接使用深圳大学 RobotPilots 战队 26 赛季的开源权重（见「参考仓库与教程」第 1 条）。
- 自训模型：训练链来自 Ultralytics（AGPL-3.0）。其余依赖与许可见「环境与依赖」「许可说明」。
