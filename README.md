...
本仓库按题分目录组织，每部分可独立构建、运行、复现；学习过程与原理问答见 `docs/notes.md`（面试复习用）。
...
继续投入需真机/定向数据支撑 → 冻结，设计决策与三级容错方案记录在 notes §17，面试可讲清"为什么停"。
...
- ROS2 踩坑（RMW/QoS/自定义消息/横屏）逐条记录在 notes §19.4，面试可直接引用。
